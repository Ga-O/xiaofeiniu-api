 ...
 *返回值形如：
 *{code: 200, msg: '1 category deleted'}, {code: 400, msg: '0 category deleted'}
 */
 ...
 * 请求参数(json格式)：{cname: 'xxx'}
 *含义： 添加新的菜品类别
 *返回值形如：
 *{code: 200, msg: '1 category added', cid: x}, {code: 400, msg: '0 category added'}
 */
router.post('/', (req, res) => {
...
router.put('/', (req,res) => {
  ...
  pool.query('UPDATE xfn_category SET ? WHERE cid = ?', [data, data.cid], (err, result) => {
    ...
    } else if (result.affectedRows == 1 && result.changeRows == 0) {//影响到1行，但修改了0行--新值与旧值一样
